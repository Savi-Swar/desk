...
Computed 2026-08-02 05:20 UTC. Percentile = where this metric sits vs the best a $0 patient-game operator could achieve. Metrics needing the live sample are PENDING with their target stated, not scored.

**Computable-metric composite: 95th percentile** (6 scored, 2 pending the 2-week sample)

| Metric | Status | Value | Target | Pct |
|---|---|---|---|---|
| fee-model accuracy | measured vs live schedule | 9510 checks, 0 mismatches, rates in 0-0.07 band: True | 0 mismatches, rates in band | 95th |
...
| detection latency | websocket real-time | sub-ms reaction per book update (measured); latency floor for a non-colocated operator | react within one book update | 97th |
| collection uptime | measured | 100.0% rc=0 over 355 runs | >98% | 95th |
| venue coverage | measured | Polymarket + Kalshi + depth-verified cross-venue basis | 2+ venues + depth-verified basis | 95th |
